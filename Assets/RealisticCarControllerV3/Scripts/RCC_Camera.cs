...
public class RCC_Camera : MonoBehaviour {
    ...
    public float orbitXSpeed, orbitYSpeed = 100f;
    ...
    private void Update() {
        ...
        if (TPSDynamic) {
            ...
            TPSDynamicInput = cameraTarget.localVelocity.magnitude / 100f;
    ...
    /// <summary>
    /// Unused another TPS method. For testing purpuses only.
    /// </summary>
    private void TPS2() {
    ...
    private void TPS() {
        ...
        if (TPSDynamic) {
            ...
            position += rotation * (Vector3.forward * TPSDynamicInput);
            position -= Vector3.up * (TPSDynamicInput / 2f);
        ...
        if (TPSDynamic)
            transform.rotation *= Quaternion.Euler(-TPSDynamicInput * 10f, 0f, 0f);
